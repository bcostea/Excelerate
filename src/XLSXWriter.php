...
class XLSXWriter {
  ...
  public function __construct() {
    ...
    is_writeable($this->tempFilename()) or $this->log("Warning: tempdir " . sys_get_temp_dir() . " not writeable, use ->setTempDir()");
    class_exists('ZipArchive') or $this->log("Error: ZipArchive class does not exist");
  ...
  protected function initializeSheet($sheet_name, $col_widths = array(), $auto_filter = false, $freeze_rows = false, $freeze_columns = false) {
    ...
    $max_cell = $this->xlsCell(self::EXCEL_2007_MAX_ROW, self::EXCEL_2007_MAX_COL); //XFE1048577
    ...
    if ($sheet->freeze_rows && $sheet->freeze_columns) {
      $sheet->file_writer->write('<pane ySplit="' . $sheet->freeze_rows . '" xSplit="' . $sheet->freeze_columns . '" topLeftCell="' . $this->xlsCell($sheet->freeze_rows, $sheet->freeze_columns) . '" activePane="bottomRight" state="frozen"/>');
      $sheet->file_writer->write('<selection activeCell="' . $this->xlsCell($sheet->freeze_rows, 0) . '" activeCellId="0" pane="topRight" sqref="' . $this->xlsCell($sheet->freeze_rows, 0) . '"/>');
      $sheet->file_writer->write('<selection activeCell="' . $this->xlsCell(0, $sheet->freeze_columns) . '" activeCellId="0" pane="bottomLeft" sqref="' . $this->xlsCell(0, $sheet->freeze_columns) . '"/>');
      $sheet->file_writer->write('<selection activeCell="' . $this->xlsCell($sheet->freeze_rows, $sheet->freeze_columns) . '" activeCellId="0" pane="bottomRight" sqref="' . $this->xlsCell($sheet->freeze_rows, $sheet->freeze_columns) . '"/>');
    } elseif ($sheet->freeze_rows) {
      $sheet->file_writer->write('<pane ySplit="' . $sheet->freeze_rows . '" topLeftCell="' . $this->xlsCell($sheet->freeze_rows, 0) . '" activePane="bottomLeft" state="frozen"/>');
      $sheet->file_writer->write('<selection activeCell="' . $this->xlsCell($sheet->freeze_rows, 0) . '" activeCellId="0" pane="bottomLeft" sqref="' . $this->xlsCell($sheet->freeze_rows, 0) . '"/>');
    } elseif ($sheet->freeze_columns) {
      $sheet->file_writer->write('<pane xSplit="' . $sheet->freeze_columns . '" topLeftCell="' . $this->xlsCell(0, $sheet->freeze_columns) . '" activePane="topRight" state="frozen"/>');
      $sheet->file_writer->write('<selection activeCell="' . $this->xlsCell(0, $sheet->freeze_columns) . '" activeCellId="0" pane="topRight" sqref="' . $this->xlsCell(0, $sheet->freeze_columns) . '"/>');
    } else {
    ...
    if (!empty($col_widths)) {
      foreach ($col_widths as $column_width) {
        $sheet->file_writer->write('<col collapsed="false" hidden="false" max="' . ($colIdx + 1) . '" min="' . ($colIdx + 1) . '" style="0" customWidth="true" width="' . floatval($column_width) . '"/>');
        $colIdx++;
      }
    }
  ...
  public function writeSheetHeader($sheet_name, array $header_types, $col_options = null) {
    ...
    if (is_bool($col_options)) {
      $this->log("Warning! passing $suppress_row=false|true to writeSheetHeader() is deprecated, this will be removed in a future version.");
  ...
  public function writeSheetRow($sheet_name, array $row, $row_options = null) {
    ...
    if (!empty($row_options)) {
      $rowHeight = isset($row_options['height']) ? floatval($row_options['height']) : 12.1;
      ...
      $sheet->file_writer->write('<row collapsed="' . ($collapsed) . '" customFormat="false" customHeight="' . ($customHt) . '" hidden="' . ($hidden) . '" ht="' . ($rowHeight) . '" outlineLevel="0" r="' . ($sheet->row_count + 1) . '">');
    } else {
  ...
  public function writeCell(BuffererWriter &$file, $row_number, $column_number, $value, $num_format_type, $cell_style_idx) {
    ...
    if (!is_scalar($value) || $value === '') { //objects, array, empty
      ...
      $file->write('<c r="' . $cell_name . '" s="' . $cell_style_idx . '" t="' . $custom_t . '"><f t="array" ref="' . $cell_name . '">' . $this->xmlspecialchars($vl) . '</f><v>0</v></c>');
  ...
  protected function styleFontIndexes() {
    ...
    foreach ($this->cell_styles as $i => $cell_style_string) {
      ...
      if (isset($style['border']) && is_string($style['border'])) {
        ...
        $style_indexes[$i]['border_idx'] = $this->add_to_list_get_index($borders, json_encode($border_value));
      }
  ...
  /*
	 ...
	 * */
  public function xlsCell($row_number, $column_number, $absolute = false) {
    $n = $column_number;
